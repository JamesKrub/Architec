                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-header bg-light-blue">
									...
									if($_SESSION['id'])
									{
										...
										$query=mysqli_query($link,$sql) or die("Can't Queryc");
                                    ...
                                    <p>
                                        ...
                                        <small>
											...
											if($_SESSION['id'])
											{
												require('connect.php');
												$sql = "select * from tz_members where id = '$id'   ";
												$query=mysqli_query($link,$sql) or die("Can't Queryd");
												$result=mysqli_fetch_array($query);
												echo"$result[email]";
											}
											?>
                    ...
                    <div class="user-panel">
                        <div class="pull-left image">
							...
							if($_SESSION['id'])
							{
								require('connect.php');
								$sql = "select * from tz_members where id = '$id'   ";
								$query=mysqli_query($link,$sql) or die("Can't Querye");
								$result=mysqli_fetch_array($query);
								echo "<img src ='../../pic/profile/profile-$result[mem_pic]' class='img-circle' alt='User Image'>";
							}
							?>
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">เอกสารจดหมายเหตุ</li>
                    </ol>
                </section>
...
$sql = "select * from tz_members where id = '$id'   ";
 $query=mysqli_query($link,$sql) or die("Can't Queryf");
...
<div class='callout callout-danger'>
	<h4>ต้องการที่จะลบเอกสารจดหมายเหตุ</h4>
	<p>
	<a class='btn btn-danger' href='showobj.php?objectid=$objectid&cfdel=1'> ยืนยัน</a>
	<a class='btn btn-default' href='showobj.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($cfdel == 1)
{
$sql = "DELETE FROM hierarchical_object WHERE obj_id = '$objectid' ";
$query=mysqli_query($link,$sql) or die("Can't Queryg");
echo "
<div id='alert-message' class='alert alert-success alert-dismissable'>
	<i class='fa fa-check'></i>
	<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
	ลบเอกสารจดหมายเหตุเสร็จสิ้น
</div>";
}

echo "<a class='btn btn-primary' href='showobjhierar.php'><i class='fa fa-plus-square fa-lg'></i> เพิ่มโฟลเดอร์</a>";
echo "<a class='btn btn-default pull-right' href='showobj_access.php'> เอกสารรอการอนุมัติ</a><br><br>";
...
if($s!='')

		{
			...
			{
       // $sql = "select * from hierarchical_object where obj_title like '%$s%' order by obj_id DESC  ";
		$sql="Select count(*) From hierarchical_object where obj_title like '%$s%' order by obj_id DESC  ";
		}
			else if ($permission =='user')
			{
        $sql = "select  count(*)  from hierarchical_object where obj_title like '%$s%' and user_id = '$id' order by obj_id DESC  ";
...
else
		{
			if(($permission == 'admin') or ($permission == 'superadmin'))
			{
          $sql = "SELECT count(*) from hierarchical_object order by obj_id DESC ";
			}
			else if ($permission =='user')
			{
                       $sql = "select  count(*) from hierarchical_object  where user_id = '$id' order by obj_id DESC ";
			}
          $query=mysqli_query($link,$sql) or die("sadasd");
          $num_rows=mysqli_num_rows($query);
		}
...
echo "<th class='col-sm-1 text-center'>รหัสเอกสารอ้างอิง</th>";
echo "<th class='col-sm-3 text-center'>ชื่อแฟ้ม</th>";
...
$sql = "select * from tz_members where id = '$id'   ";
 $query=mysqli_query($link,$sql) or die("Can't Querya");
 $num_rows=mysqli_num_rows($query);
...
if($s!='')
{
	...
	{
		$sql = "select * from hierarchical_object where obj_title like '%$s%'";
	}
	else if ($permission =='user')
	{
		$sql = "select * from hierarchical_object where obj_title like '%$s%' and user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Querya");
	$num_rows=mysqli_num_rows($query);
}
else
{
	if(($permission == 'admin') or ($permission == 'superadmin') or ($permission =='editor'))
	{
		$sql = "select * from hierarchical_object";
	}
	else if ($permission =='user')
	{
		$sql = "select * from hierarchical_object  where user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Queryb");
	$num_rows=mysqli_num_rows($query);
}
...
while($result=mysqli_fetch_array($rs)){ //วนรอบแสดงข้อมูล
	...
	$sql2 = "SELECT * FROM archive_pic WHERE obj_refcode = '$objrefcode' ORDER BY obj_cover DESC limit 0,1  ";
	...
	echo "<td><img src ='../../pic/big/$objref/$picname' width='250'></td>";
	...
	{
    echo "<button type='button ' class='btn btn-primary disabled'>การจัดการ</button>
	...
	{
    echo "<button type='button ' class='btn btn-primary '>การจัดการ</button>
    ...
    <li><a href='edit.php?objectid=$result[obj_id]&refcode=$result[obj_refcode]'>แก้ไข</a></li>
