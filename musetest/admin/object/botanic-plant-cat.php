            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">ประเภทพรรณไม้</li>
                    </ol>
                </section>
...
$sql = "UPDATE botanic_plant_category SET `cat1_name` = '$catname'  WHERE `botanic_plant_category`.`cat1_id` = '$catid' LIMIT 1 ;";
$query=mysqli_query($link,$sql) or die("Can't Query");
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='botanic-plant-cat.php?catid=$catid&del=2'> ยืนยัน</a>
	<a class='btn btn-default' href='botanic-plant-cat.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($del == 2)
{
$sql = "DELETE FROM botanic_plant_category WHERE cat1_id = '$catid' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$sql2 = "DELETE FROM botanic_plant_category WHERE cat1_parent = '$catid' ";
...
$sql = "INSERT INTO botanic_plant_category (`cat1_id` ,`cat1_name`,`cat1_parent` )VALUES (NULL, '$catname','$parentid');";
$query=mysqli_query($link,$sql) or die("Can't Query");
...
echo "<form class='form-horizontal' role='form' name='form1' action='botanic-plant-cat.php' method='post'>";
...
//echo "<br>";
$sql = "select  count(*) from botanic_plant_category  where cat1_parent = '0' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$num_rows=mysqli_num_rows($query);
...
////////// End Check User ///////
          ...
          $sql = "select * from botanic_plant_category where cat1_parent = '0'  order by cat1_id DESC LIMIT $start , $p_size ";
          $query=mysqli_query($link,$sql) or die("Can't Query");
          $num_rows=mysqli_num_rows($query);

//$sql="Select * From botanic_plant_category LIMIT $start , $p_size";
...
while($c1 = mysqli_fetch_array($rs)) {
	echo "<tr>";
		echo "<form action='botanic-plant-cat.php' method='post'>";
			...
			echo "<td class='text-center'>";
				...
				echo "<a class='btn btn-link' href='botanic-plant-cat.php?catid=".$c1['cat1_id']."&del=1'><img src='images/icon_del2.png'></a>";
			echo "</td>";
		echo "</form>";
	echo "</tr>";
	...
	$cmd = "
		SELECT *
		FROM `botanic_plant_category_lv2` AS c2
		LEFT JOIN `botanic_plant_category` AS c1 ON c2.ac1_id = c1.cat1_id
	...
	if(mysqli_num_rows($queryCL2) != 0) {
		...
		while($c2 = mysqli_fetch_assoc($queryCL2)) {
			echo "<tr>";
				echo "<form action='model/funcCategory.php' method='post'>";
					...
					echo "<td class='text-center'>";
						...
						echo "<input type='hidden' name='table' value='botanic_plant'>";
			...
			$cmd = "
				SELECT *
				FROM `botanic_plant_category_lv3` AS c3
				LEFT JOIN `botanic_plant_category_lv2` AS c2 ON c3.ac2_id = c2.ac2_id
            ...
            $(function() {
				...
				$("#ca").on('change', function() {
					...
					if($(this).val() != 0) {
						...
						level_2_add += "<input type='hidden' name='table' value='botanic_plant'>";
						...
						$.ajax(
							...
							{url: "model/getCategory.php?table=plant&level=2&id="+ $(this).val(),
							...
							if(fetch.length != 0) {
								...
								level_2_list += "<input type='hidden' name='table' value='botanic_plant'>";
