...
$sql = "select * from tz_members where id = '$id'   ";
 $query=mysqli_num_rows ($link,$sql) or die("Can't Query");
...
//        $query = mysqli_num_rows"SELECT * FROM `news_upload` WHERE `obj_id` = '".$objid."'");
         $query = mysqli_num_rows"SELECT * FROM `news_upload` WHERE `obj_id` = '".$_GET['newsid']."'");
		if(mysqli_num_rows($query) != 0) {
			...
			<div class='row'>
					...
					while($row = mysql_fetch_assoc($query)) {
						echo "
							<div class='col-md-2'>
                                <img src='../../pic/news_upload/".$row['bpu_file']."' style='border: 1px solid #ccc;'>
								<br><br>";
