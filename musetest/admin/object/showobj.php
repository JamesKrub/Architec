...
{
$sql = "DELETE FROM archive_object WHERE obj_id = $objectid ";
...
// echo "<a class='btn btn-primary' href='addobj-1page.php'><i class='fa fa-plus-square fa-lg'></i> เพิ่มข้อมูล</a>";
echo "<a class='btn btn-primary' href='showobjhierar.php'><i class='fa fa-plus-square fa-lg'></i> เพิ่มข้อมูล</a>";
...
if($s!='')

		{
			...
			{
       // $sql = "select * from archive_object where obj_title like '%$s%' order by obj_id DESC  ";
		$sql="Select count(*) From archive_object where obj_title like '%$s%' order by obj_id DESC  ";
		}
			else if ($permission =='user')
			{
        $sql = "select  count(*)  from archive_object where obj_title like '%$s%' and user_id = '$id' order by obj_id DESC  ";

			}
        $query=mysqli_query($link,$sql) or die("Csdasdan't Query");
        $num_rows=mysqli_num_rows($query);
		}
else
		{
			if(($permission == 'admin') or ($permission == 'superadmin'))
			{
          $sql = "select count(*) from archive_object order by obj_id DESC ";
			}
			else if ($permission =='user')
			{
                       $sql = "select  count(*) from archive_object  where user_id = '$id' order by obj_id DESC ";
			}
...
if($s!='')
{
	...
	{
		$sql = "select * from archive_object where obj_title like '%$s%'";
	}
	else if ($permission =='user')
	{
		$sql = "select * from archive_object where obj_title like '%$s%' and user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Querya");
	$num_rows=mysqli_num_rows($query);
}
else
{
	...
	{
		$sql = "select * from archive_object  where user_id = '$id'";
	}
