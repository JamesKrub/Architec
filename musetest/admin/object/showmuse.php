            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">วัตถุจัดแสดง</li>
                    </ol>
                </section>
...
echo "<a class='btn btn-primary' href='addmuse-1page.php'><i class='fa fa-plus-square fa-lg'></i> เพิ่มข้อมูล</a>";
echo "<a class='btn btn-default pull-right' href='showmuse_access.php'> วัตถุรอการอนุมัติ</a><br><br>";
...
if($s!='')

		{
			...
			{
       // $sql = "select * from muse_object where obj_title like '%$s%' order by obj_id DESC  ";
		$sql="Select count(*) From muse_object where obj_title like '%$s%' order by obj_id DESC  ";
		}
			else if ($permission =='user')
			{
        $sql = "select  count(*)  from muse_object where obj_title like '%$s%' and user_id = '$id' order by obj_id DESC  ";
...
else
		{
			if(($permission == 'admin') or ($permission == 'superadmin'))
			{
          $sql = "select count(*) from muse_object order by obj_id DESC ";
			}
			else if ($permission =='user')
			{
                       $sql = "select  count(*) from muse_object  where user_id = '$id' order by obj_id DESC ";
			}
          $query=mysqli_query($link,$sql) or die("Can't Query");
          $num_rows=mysqli_num_rows($query);
		}
...
if($s!='')
{
	...
	{
		$sql = "select * from muse_object where obj_title like '%$s%' and user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Query");
	$num_rows=mysqli_num_rows($query);
}
...
//				$picname=$result2['pic_name'];
	...
	echo "<td><img src ='../../pic/thumbmuse/$objref/$picname' width='250'></td>";
    ...
    <li><a href='showmuse.php?objectid=$result[obj_id]&del=obj'>ลบ</a></li>
    <li class='divider'></li>";
