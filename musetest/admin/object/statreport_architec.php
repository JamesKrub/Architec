            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">สถิติสถาปัตยกรรม</li>
                    </ol>
                </section>
...
if($s!='')
{
	if(($permission == 'admin') or ($permission == 'superadmin'))
	{
		$sql = "select * from architec_object where archObj_Title like '%$s%'";
	}
	else if ($permission =='user')
	{
		$sql = "select * from architec_object where archObj_Title like '%$s%' and user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Query");
	$num_rows=mysqli_num_rows($query);
}
else
{
	...
	{
		$sql = "select * from architec_object  where archUser_Id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Query");
	$num_rows=mysqli_num_rows($query);
}
...
while($result=mysqli_fetch_array($rs)){ //วนรอบแสดงข้อมูล
	...
	$sql3 = "SELECT * FROM architec_pic WHERE archObj_Refcode = '$objrefcode'  ";
	...
	echo "<td><img src ='../../pic/big_architec/$objref/$picname' width='250'></td>";
	...
	echo "<td class='text-center'>".$result['archObj_Countdownload']."</td>";
$i=$i+1;
}
