        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
						เอกสารรอการอนุมัติข้อมูลพรรณไม้
                    </h1>
                    <ol class="breadcrumb">
                        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
                        <li><a href="index.php"> ข้อมูลพรรณไม้</a></li>
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='show-plant.php?objectid=$objectid&cfdel=1'> ยืนยัน</a>
	<a class='btn btn-default' href='show-plant.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($cfdel == 1)
{
$sql = "DELETE FROM botanic_plant_object WHERE obj_id = $objectid ";
$query=mysqli_query($link,$sql) or die("Can't Query");
echo "
...
echo "<a class='btn btn-default pull-right' href='show-plant.php'>ข้อมูลพรรณไม้</a><br><br>";
...
if($s!='')

		{
			...
			{
       // $sql = "select * from botanic_plant_object where obj_title like '%$s%' order by obj_id DESC  ";
		$sql="Select count(*) From botanic_plant_object where obj_title like '%$s%' order by obj_id DESC  ";
		}
			else if ($permission =='user')
			{
        $sql = "select  count(*)  from botanic_plant_object where obj_title like '%$s%' and user_id = '$id' order by obj_id DESC  ";
...
else
		{
			if(($permission == 'admin') or ($permission == 'superadmin'))
			{
          $sql = "select count(*) from botanic_plant_object order by obj_id DESC ";
			}
			else if ($permission =='user')
			{
                       $sql = "select  count(*) from botanic_plant_object  where user_id = '$id' order by obj_id DESC ";
			}
          $query=mysqli_query($link,$sql) or die("Can't Query");
          $num_rows=mysqli_num_rows($query);
		}
...
if($s!='')
{
	if(($permission == 'admin') or ($permission == 'superadmin'))
	{
		$sql = "select * from botanic_plant_object where obj_title like '%$s%'";
	}
	else if ($permission =='user')
	{
		$sql = "select * from botanic_plant_object where obj_title like '%$s%' and user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Query");
	$num_rows=mysqli_num_rows($query);
}
else
{
	if(($permission == 'admin') or ($permission == 'superadmin'))
	{
		$sql = "select * from botanic_plant_object where obj_access != '1'";
	}
	else if ($permission =='user')
	{
		$sql = "select * from botanic_plant_object  where user_id = '$id'";
	}
	$query=mysqli_query($link,$sql) or die("Can't Query");
	$num_rows=mysqli_num_rows($query);
}
...
while($result=mysqli_fetch_array($rs)){ //วนรอบแสดงข้อมูล
	...
	echo "<td class='text-center'><a href=\"edit-plant.php?objectid=$result[obj_id]&refcode=$result[obj_refcode]\"><img src='images/edit_icon.png'></a> |
	<a href=\"show-plant.php?objectid=$result[obj_id]&del=obj\"><img src='images/icon_del.gif'></a></td>";
