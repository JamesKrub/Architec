        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                    <h1>
						แก้ไขข้อมูลภูมิปัญญา
                    </h1>
                    <ol class="breadcrumb">
                        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
                        <li><a href="show-idea.php"><i class="fa fa-dashboard"></i> ข้อมูลภูมิปัญญา</a></li>
                        <li class="active">แก้ไขข้อมูลภูมิปัญญา</li>
                    </ol>
                </section>
...
<div id='alert-message' class='alert alert-success alert-dismissable'>
	...
	แก้ไขข้อมูลข้อมูลภูมิปัญญาเสร็จสิ้น
</div>";
}

//fetching data in descending order (lastest entry first)
 $sql = "select * from archive_object  where obj_id = '$objectid'";
         $query=mysqli_query($link,$sql) or die("555s");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
$query = mysqli_query($link,"SELECT * FROM botanic_idea_object WHERE obj_id = '".$_GET['objectid']."'");
			...
			<div class='box box-primary'>
				<div class='box-body'>
					<div class='form-group'>
						<label for='code' class='col-sm-2 control-label'>รหัสภูมิปัญญา</label>
						<div class='col-sm-10'>
					...
					<div class='form-group'>
						...
						<div class='col-sm-10'>
							<select name='cate' class='form-control' id='cate_1'>
								...
								";
								$query = mysqli_query($link,"SELECT * FROM botanic_idea_category");
						...
						echo "
								...
								<div class='col-sm-9'>
									<select name='cate_2' class='form-control' id='cate_2'>";
										...
										$query = mysqli_query($link,"SELECT * FROM botanic_idea_category_lv2 WHERE ac1_id = '$row[obj_cate]'");
										while($cate = mysqli_fetch_array($query)) {
						...
						echo "
								...
								<div class='col-sm-8'>
									<select name='cate_3' class='form-control' id='cate_3'>";
										echo "<option value='0'>ไม่ระบุ</option>";
										$query = mysqli_query($link,"SELECT * FROM botanic_idea_category_lv3 WHERE ac2_id = '$row[obj_cate2]'");
										while($cate = mysqli_fetch_array($query)) {
...
echo "<div class='box box-primary'>"; // Start box-primary
    ...
    echo "<a href='edit-idea.php?download=1&objectid=$objectid&refcode=$refcode'><img src='images/icon_set3.png'> </a>อนุญาตให้ดาวน์โหลด";
    echo "<a href='edit-idea.php?deldownload=1&objectid=$objectid&refcode=$refcode'><img src='images/icon_del2.png'> </a>ลบ <Br><br>";
    }
    else if(($downloadfile != '') && ($download == '1'))
	{
    echo "<label> ไฟล์สำหรับ Download <a href='../../pic/download/$downloadfile' target='_blank'> $downloadfile </a></label> <br>";
    echo "<a href='edit-idea.php?download=0&objectid=$objectid&refcode=$refcode'><img src='images/icon_set2.png'> </a>อนุญาตให้ดาวน์โหลด ";
    echo "<a href='edit-idea.php?deldownload=1&objectid=$objectid&refcode=$refcode'><img src='images/icon_del2.png'> </a>ลบ <Br><br>";
    }
    ...
    echo "</form>";
		...
		$ul_query = mysqli_query($link,"SELECT * FROM `botanic_idea_upload` WHERE `obj_id` = '".$objid."'");
		echo "<div class='row'>";
		while($row = mysqli_fetch_assoc($ul_query)) {
			...
			$type = "idea";
			echo "
                <img src='../../pic/botanic_idea_upload/".$refcode."/".$row['bpu_file']."'><br>
				<a target='_blank' href='../../pic/botanic_idea_upload/".$refcode."/".$row['bpu_file']."'>
...
if($delpic == 1)
		{
	      $sql = "DELETE FROM botanic_idea_pic WHERE pic_id = '$picid' ";
...
// ปรับค่าให้ obj_cover อื่นๆให้มีค่าเป็น 0 เพื่อที่จะให้เหลือค่า 1 ค่าเดียว
 $sql0 = "UPDATE botanic_idea_pic SET `obj_cover`='0' WHERE `botanic_idea_pic`.`obj_refcode` ='$refcode' AND `botanic_idea_pic`.`pic_id` !='$picid'";
 $query0=mysqli_query($link,$sql0) or die("Can't Query-0");

 $sql = "UPDATE botanic_idea_pic SET `obj_cover`='1' WHERE `botanic_idea_pic`.`pic_id` ='$picid'";
...
{
//echo "สั่งเปิดรูป";
 $sql = "UPDATE botanic_idea_pic SET `pic_open`='0' WHERE `botanic_idea_pic`.`pic_id` ='$picid'";
 $query=mysqli_query($link,$sql) or die("Can't Query-1");
}
else if($openpic == '0')
{
//echo "สั่งปิดรูป";
 $sql = "UPDATE botanic_idea_pic SET `pic_open`='1' WHERE `botanic_idea_pic`.`pic_id` ='$picid'";
 $query=mysqli_query($link,$sql) or die("Can't Query-1");
}
	...
	echo "<div class='box-body'>"; // Start box-body
	echo "<div>
			<a class='btn btn-primary' href=\"JavaScript:newPopup('upload-idea/index.php?dir=$refcode&objectid=$objectid&refcode=$refcode');\">
...
if($updatepic == '1')
{
$sql5 = "SELECT * FROM `botanic_idea_pic` WHERE obj_refcode = '$refcode' ";
...
######## UPload FILE###########
 $target_path = "pic/big-idea/";
 $target_path_thumb = "pic/thumb-idea/";
...
//$dest = "pic/big-idea/"."thumb-".$myfile;
//rename($target_path, $dest);

$original = "../../pic/big-idea/".$myfile;
$dest = "../../pic/thumb-idea/".$myfile;
copy($original,$dest);

$thumb = "thumb-".$myfile;
$mythumb = "../../pic/thumb-idea/".$thumb;
//echo "$thumb --> $mythumb";
rename($dest, $mythumb);

//////////////////////////////////////////

	$sql = "INSERT INTO botanic_idea_pic (`pic_id` ,`pic_name` ,`obj_id` ,`obj_refcode` )VALUES ( NULL, '$myfile', 0, '$refcode')";
...
echo "<tr>";
 $sql3 = "SELECT * FROM `botanic_idea_pic` WHERE obj_refcode = '$refcode' ORDER BY listorder ";
	...
	//	echo "x $num_rows3 x <br>";
	     for ($i=0; $i<$num_rows3; $i++) {
				 ...
				 if($line < 3)
			 {
                ...
                // echo "<img src ='../../pic/thumb-idea/thumb-$result3[pic_name]' width='50'> $result3[pic_id] $result3[obj_refcode]$result3[pic_name] <br>";

				  if(($filetype == 'jpg') or ($filetype =='jpeg') or ($filetype =='png'))
				  {
				  			echo "<a href=\"../../pic/big-idea/$refcode/$result3[pic_name]\" data-gallery>
							<img src ='../../pic/big-idea/$refcode/$result3[pic_name]' width='200'></a> <br>";
				  }
				  else
				  if($filetype =='mp4')
				  {
				  			//echo "pic/big-idea/$refcode/$result3[pic_name]";
							echo "<video width='200'  controls>
  								  <source src='../../pic/big-idea/$refcode/$result3[pic_name]' type='video/mp4'>
  								  <object data='../../pic/big-idea/$refcode/$result3[pic_name]' width='200' >
				  ...
				  else if($filetype =='mp3')
				  {
				  		...
				  		echo "<audio width='200' controls>
  				  		<source src='../../pic/big-idea/$refcode/$result3[pic_name]' type='audio/mpeg'>
  			     		<embed src='../../pic/big-idea/$refcode/$result3[pic_name]' width='200'>
						</audio>";
                  		echo "<br>";
				  }
				  ...
				  {
				  		//echo "$result3[pic_name] <br>";
				  		echo "<a href ='../../pic/big-idea/$refcode/$result3[pic_name]' target='_blank'><img src='images/pdf-icon.png' width='150'></a>";
                  		echo "<br>";
				  }


			echo "
			<a href=\"JavaScript:newPopupdetail('zoom/picdetail-botanic.php?type=idea&picid=$result3[pic_id]&objectid=$objid&refcode=$refcode');\"> <img src='images/edit_icon2.png'></a>
			<a href ='edit-idea.php?picid=$result3[pic_id]&delpic=1&objectid=$objid&refcode=$refcode'> <img src ='images/icon_del2.png'> </a>";

			if(($filetype == 'jpg') or ($filetype =='jpeg') or ($filetype =='png'))
			{
				if($cover == '1')
				{
					echo "<a href ='edit-idea.php?picid=$result3[pic_id]&setpic=1&objectid=$objid&refcode=$refcode'> <img src ='images/icon_set2.png'> </a>"; //Checked
				}
				else
				{
					echo "<a href ='edit-idea.php?picid=$result3[pic_id]&setpic=1&objectid=$objid&refcode=$refcode'> <img src ='images/icon_set3.png'> </a>"; //Unchecked
				}
			}

			if($result3[pic_open] == '0')
			{
			echo "<a href ='edit-idea.php?picid=$result3[pic_id]&openpic=0&objectid=$objid&refcode=$refcode'><img src='images/eye-open.png'></a>";
			}
			else
			{
			echo "<a href ='edit-idea.php?picid=$result3[pic_id]&openpic=1&objectid=$objid&refcode=$refcode'><img src='images/eye-close.png'></a>";
			}
			...
			$line = $line+1;
			 ...
			 else
             {
				 echo "<td align='center'>";
                //echo "<img src ='../../pic/thumb-idea/thumb-$result3[pic_name]' width='50'> $result3[pic_id] $result3[obj_refcode]$result3[pic_name] <br>";
				// echo "        <a href=\"pic/big-idea/$refcode/$result3[pic_name]\"   media=\"gallery\" id=\"tre\"  class=\"pirobox_gall\" title=\"\">
				//<img src ='../../pic/big-idea/$refcode/$result3[pic_name]' width='150'></a>
				//<br>
				...
				//<a href ='edit-idea.php?picid=$result3[pic_id]&delpic=1&objectid=$objid&refcode=$refcode'> <img src ='images/icon_del.gif'> </a>";
