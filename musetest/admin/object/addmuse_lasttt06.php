...
//}
  ...
  // echo "sub2 = ".$categorysub2."<br> sub3 = ".$categorysub3;
	...
	else // if all the fields are filled (not empty)
	{
		...
		//$result=mysqli_query($link,"INSERT INTO users(obj_refcode,obj_title,obj_category) VALUES('$refcode','$title','$category')");

		//$sql = " INSERT INTO rcy_object (`obj_id` ,`obj_refcode` ,`obj_title` ,`obj_category` )VALUES ('', '$refcode', '$title', '$category');";
		$sql = "INSERT INTO muse_object
 ...
 if (!mysqli_query($link,$sql))
   {
    echo("พังที่นี่นะแก้ด้วย    " . mysqli_error($link) . "<br>");
    	...
    	//กำหนดค่าการเชื่อมต่อ DB api
	$host1 = "emuseum_db";
	$username1 = "root";
	$dbname1 = "culture_api";
	//$password2 = "";	
	$password3 = "heavygeese24"; 
	
	date_default_timezone_set('Asia/Bangkok');	

	$link1 = mysqli_connect($host1,$username1,$password3,$dbname1);
	...
	mysqli_commit($link1);    
       ...
       $sql1 = "INSERT INTO `culture_api`.`itemsnew` (`culture_api`.`itemsnew`.`ownermuseum_code`, `culture_api`.`itemsnew`.`obj_id`, `culture_api`.`itemsnew`.`obj_refcode`, `culture_api`.`itemsnew`.`obj_title`, `culture_api`.`itemsnew`.`obj_titleTH`, `culture_api`.`itemsnew`.`obj_titleEN`, `culture_api`.`itemsnew`.`obj_descTH`, `culture_api`.`itemsnew`.`obj_descEN`, `culture_api`.`itemsnew`.`obj_excepTH`, `culture_api`.`itemsnew`.`obj_excepEN`, `culture_api`.`itemsnew`.`obj_datecreate`, `culture_api`.`itemsnew`.`obj_level`, `culture_api`.`itemsnew`.`obj_extent`, `culture_api`.`itemsnew`.`obj_creator`, `culture_api`.`itemsnew`.`obj_bio`, `culture_api`.`itemsnew`.`obj_dateacc`, `culture_api`.`itemsnew`.`obj_history`, `culture_api`.`itemsnew`.`obj_acquis`, `culture_api`.`itemsnew`.`obj_scope`, `culture_api`.`itemsnew`.`obj_appraisal`, `culture_api`.`itemsnew`.`obj_accruals`, `culture_api`.`itemsnew`.`obj_arrangement`, `culture_api`.`itemsnew`.`obj_legal`, `culture_api`.`itemsnew`.`obj_condition`, `culture_api`.`itemsnew`.`obj_copyright`, `culture_api`.`itemsnew`.`obj_lang`, `culture_api`.`itemsnew`.`obj_physicals`, `culture_api`.`itemsnew`.`obj_aids`, `culture_api`.`itemsnew`.`obj_location`, `culture_api`.`itemsnew`.`obj_existence`, `culture_api`.`itemsnew`.`obj_related`, `culture_api`.`itemsnew`.`obj_associated`, `culture_api`.`itemsnew`.`obj_pubnote`, `obj_note`, `obj_date`, `obj_category`, `obj_access`, `user_id`, `obj_keyword`, `obj_approve`, `obj_relation`, `obj_display`, `obj_location_display`, `obj_existence_display`, `obj_creator_display`, `obj_bio_display`, `obj_dateacc_display`, `obj_history_display`, `obj_acquis_display`, `obj_approver`, `obj_isFeatured`, `obj_MapImageID`, `obj_x`, `obj_y`, `obj_upduser`, `obj_upddate`, `obj_countqr`, `obj_count`, `cat1_id`, `cat1_name`, `cat1_type`, `pic_id`, `pic_name`, `pic_nameTH`, `pic_nameEN`, `pic_descTH`, `pic_descEN`, `pic_path`, `pic_width`, `pic_height`, `pic_pathS`, `pic_pathM`, `pic_pathL`, `pic_type`, `pic_isThumbnail`, `updUser`, `updDate`)
        VALUES ('1', NULL, '$refcode', '$title', '$title', '$titleeng', NULL, NULL, NULL, NULL, '$datecreate', '$level', '$extent', '$creator', '$bio', '$dateacc','$history', '$acquis', '$scope','$appraisal', '$accruals',  '$arrangement', '$legal', '$condition', '$copyright', '$lang', '$physicals', '$aids', '$location', '$existence', '$related','$associated', '$pubnote', '$note', '$data', '$category', '$access', '$id', '$keyword', '$approve', '$relation',  '$display', '$location_display', '$existence_display', '$creator_display', '$bio_display', '$dateacc_display',  '$history_display', '$acquis_display', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);  ";
        
// edit by day
 if (!mysqli_query($link1,$sql1))
   {
    echo("พังที่นี่นะแก้ด้วย 1   " . mysqli_error($link1) . "<br>");
 }
  ...
  $query=mysqli_query($link,$sql) or die("Can't Query6");
   $num_rows=mysqli_num_rows($query);
		...
		echo "<a class='btn btn-primary' href='editmuse.php?objectid=$objectid'> กลับหน้าแสดงผล</a>";
	}
