...
$parenth1_forlder = $_REQUEST['parents'];
$parant_hc1 = $_REQUEST['parant_hc1'];
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-header bg-light-blue">
									...
									if($_SESSION['id'])
									{
										...
										$query=mysqli_query($link,$sql) or die("sss");
                                    ...
                                    <p>
                                        ...
                                        <small>
											...
											if($_SESSION['id'])
											{
												require('connect.php');
												$sql = "select * from tz_members where id = '$id'   ";
												$query=mysqli_query($link,$sql) or die("Css");
												$result=mysqli_fetch_array($query);
												echo"$result[email]";
											}
											?>
                    ...
                    <div class="user-panel">
                        <div class="pull-left image">
							...
							if($_SESSION['id'])
							{
								require('connect.php');
								$sql = "select * from tz_members where id = '$id'   ";
								$query=mysqli_query($link,$sql) or die("Cdasdasd");
								$result=mysqli_fetch_array($query);
								echo "<img src ='../../pic/profile/profile-$result[mem_pic]' class='img-circle' alt='User Image'>";
							}
							?>
...
<!--------------   Process ----------------->
	...
	######  Start Tab ##########
echo "
<form class='form-horizontal' role='form' name='form1' method='post' action='add.php' enctype='multipart/form-data'>
...
<div class='form-group'>
	<label class='col-sm-2 control-label' >โฟลเดอร์จัดเก็บ*</label>
	<div class='col-sm-10'>
<?php
 include('conf/connectdb.php.inc'); ?>
	<?php
echo "<select class='form-control' name='category_hc' id='cate_h1'>";

	$sql = "select * from hierarchical_category  where folder1_id ='$parenth1_forlder' ";
	$query=mysqli_query($link,$sql) or die("ss");
	$num_rows=mysqli_num_rows($query);
	// $result=mysqli_fetch_array($query);
	foreach ($query as $result) {
		if($category_hc ==  $result[folder1_id])
		{
			echo "<option value='$result[folder1_id]' selected>$result[folder1_name]</option>";
		}
		else
		{
	...
	// Sub Category //
		$parent = $result[folder1_id];
		$sql5 = "select * from hierarchical_category  ";
		$query5=mysqli_query($link,$sql5) or die("ss");
...
echo "
<div id='form_cate_h2' class='form-group'></div>
<div id='form_cate_h3' class='form-group'></div>
";
	?>
	<?php
echo "
...
<div class='form-group'>
	...
	<div class='col-sm-5'>
		<input id='datepicker' data-date-format='yyyy-mm-dd' class='form-control' type='text' name ='datecreate' value='$datecreate'>
	</div>
</div>
...
######### End Of Tab  Pic 1 ############
	...
	<!---- End of  Picture  -------->
		...
		$("#hc_1").on('change', function() {
			...
			$.ajax({
			url: 'model/getCategory.php',
			data : {'table': type , 'level': level2 ,'id': id },
	...
	$("#cate_hc1").on('change', function() {
		var list_2 = "";
		var list_3 = "";
		$("#form_cate_hc2").html(list_2);
		$("#form_cate_hc3").html(list_3);
		var id = $(this).val();
		var level2 = '2';

		$.ajax({
		url: 'model/getHierarchical.php',
		data : {'table': type , 'level': level2 ,'id': id },
		...
		success : function(result){
			...
			if(fetch.length > 0) {
				list_2 += "<label for='cate_h2' class='col-sm-3 control-label'>ประเภทระดับ 2</label>";
				list_2 += "<div class='col-sm-9'>";
				list_2 += "<select name='cate_hc2' class='form-control' id='cate_hc2'>";
				list_2 += "<option value='0'>ไม่ระบุ</option>";
				$.each(fetch, function(key, value) {
					list_2 += "<option value='"+value.id+"'>" + value.name + "</option>";
				});
				...
				$("#cate_2").on('change', function() {
					var list_3 = "";
					$("#form_cate_hc3").html(list_3);
					$.ajax({url: "model/getHierarchical.php?table="+type+"&level=3&id="+ $(this).val(), success: function(xx){
						var ff = $.parseJSON(xx);

						if(ff.length > 0) {
							list_3 += "<label for='cate_hc3' class='col-sm-4 control-label'>ประเภทระดับ 3</label>";
							list_3 += "<div class='col-sm-8'>";
							list_3 += "<select name='cate_hc3' class='form-control' id='cate_3'>";
							list_3 += "<option value='0'>ไม่ระบุ</option>";
							$.each(ff, function(key, value) {
								list_3 += "<option value='"+value.id+"'>" + value.name + "</option>";
							});
