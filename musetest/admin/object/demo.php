...
if($_POST['submit']=='Login')
{
	...
	if(!count($err))
	{
		...
		//$row = mysql_fetch_assoc(mysqli_query($link,"SELECT id,usr FROM tz_members WHERE usr='{$_POST['username']}' AND pass='".md5($_POST['password'])."'"));
		$row = mysql_fetch_assoc(mysqli_query($link,"SELECT id,usr FROM tz_members WHERE usr='{$_POST['username']}' AND pass='{$_POST['password']}' "));
...
#mysqli_query($link,"	INSERT INTO tz_members(usr,pass,email,regIP,dt,permission)
	#					VALUES(
		#
			#				'".$_POST['username']."',
				#			'".md5($pass)."',
...
<head>
    ...
    <title>ระบบบริหารจัดการเอกสารจดหมายเหตุ</title>
...
<!-- Page Content -->
    <div class="container">
        ...
        <header class="jumbotron hero-spacer">
            <h1>ระบบบริหารจัดการเอกสารจดหมายเหตุ</h1>
