...
$sql3 = "select bg_path from muse_bg  ";
$query3 = mysqli_query($link2,$sql3) or die("Can't Query");
$result3 = mysqli_fetch_array($query3);
$bg_path = $result3['bg_path'];
...
$sql2 = "UPDATE muse_bg SET `bg_name` = '$archivename', `bg_desc` = '$archivedesc', `bg_entry` = '$archiveentry',
`bg_lat` = '$archivelat',`bg_lon` = '$archivelon',`bg_number` = '$archivenumber',`bg_tambon` = '$archivetambon',
`bg_ampher` = '$archiveampher',`bg_city` = '$archivecity',`bg_postcode` = '$archivepostcode',`bg_tel` = '$archivetel',
`bg_email` = '$archiveemail',`bg_website` = '$archivewebsite',`bg_update` = '$timestring'
...
VALUES ('1', '$archivename','$archivedesc','$archiveentry','$archivelat','$archivelon','$archivenumber',
...
echo "<div class='form-group'>";
echo "<label class='col-sm-2 control-label'>โทรศัพท์</label>";
echo "<div class='col-sm-10'>";
...
//ส่งข้อมูล นิทรรศการ
echo "<div class='box box-primary'>"; // Start box-primary
echo "<form action='http://www.museumspool.com/adt_anurak/index.php/Adapter/update_museum' target='_blank' method='post'>";

echo "<div class='box-body'>"; // Start box-body

echo "<div class='form-group'>";
        //   echo "$domain_name"; 
         $uri1 = $_SERVER['REQUEST_URI']; // $uri == example.com/index
 $exploded_uri1 = explode('/', $uri1); //$exploded_uri ==     array('example.com','index')
  $domain_name1 = $exploded_uri1[1]; 

//         $bg_name6 = $result6['bg_name'];         
//echo "<label class='col-sm-2 control-label'>ส่งข้อมูล $domain_name1  </label>";
require('connect_api.php');        
  $sql6 = "SELECT culture_api.muse_bg.bg_id FROM culture_api.muse_bg WHERE culture_api.muse_bg.bg_path = '$domain_name1' ";
...
//         $num_rows5=mysqli_num_rows($query5);  
       ...
       $bg_name6 = $result6['bg_name'];    
//echo "<label class='col-sm-2 control-label'>ส่งข้อมูล1 $domain_name1 $sql6 </label>";        
echo "<div class='col-sm-10'>";
//echo "<input class='form-control' type='text' name ='archivename' value='$archivename'>";
 echo "<input type='hidden' name='action'value='Item'/>";
 echo "<input name='museum_code' type='hidden' id='museum_code' value='$bg_id6'>   "; 
...
echo "<div class='box-footer'>"; // Start box-footer
echo "<input type='hidden' name='editdetail' value='1'><button type='submit' name='submit' class='btn btn-primary'><i class='fa fa-upload fa-lg'></i>ส่งข้อมูล พิพิธภัณฑ์</button>";
echo "</div>"; // End box-footer
