                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-header bg-light-blue">
									...
									if($_SESSION['id'])
									{
										...
										$query=mysqli_query($link,$sql) or die("Can't Quหหหery");
                                    ...
                                    <p>
                                        ...
                                        <small>
											...
											if($_SESSION['id'])
											{
												require('connect.php');
												$sql = "select * from tz_members where id = '$id'   ";
												$query=mysqli_query($link,$sql) or die("Can't Queหry");
												$result=mysqli_fetch_array($query);
												echo"$result[email]";
											}
											?>
                    ...
                    <div class="user-panel">
                        <div class="pull-left image">
							...
							if($_SESSION['id'])
							{
								require('connect.php');
								$sql = "select * from tz_members where id = '$id'   ";
								$query=mysqli_query($link,$sql) or die("Can't Qหหuery");
								$result=mysqli_fetch_array($query);
								echo "<img src ='../../pic/profile/profile-$result[mem_pic]' class='img-circle' alt='User Image'>";
							}
							?>
...
$sql = "INSERT INTO hierarchical_category (`folder1_id` ,`folder1_name`,`folder1_parent` )VALUES (NULL, '$folder1_name','$folder1_parent');";
$query=mysqli_query($link,$sql) or die("Caหหn't Query");



echo "<a href='addobj-1page.php?parents=$folderid&folder1_name=$folder1_name&folder1_parent=$folder1_parent' type='button' class='btn btn-primary'>";
echo "sdasdasd";
echo "</a>";

// header("Location: ");

// header("Refresh:0; url=addobj-1page.php?parents=$folderid&folder1_name=$folder1_name&folder1_parent=$folder1_parent");
// header("Refresh:0; url=page2.php");
// exit;
 ?>
<script>
	window.location.href  ="addobj-1page.php?parents="+.$folderid.+"&folder1_name="+.$folder1_name.+"&folder1_parent="+.$folder1_parent" " ;
</script>
 <?php
...
$sql = "UPDATE hierarchical_object SET `folder1_name` = '$folder1_name'  WHERE `hierarchical_object`.`folder1_id` = '$folderid' LIMIT 1 ;";
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='showobjhierar.php?folder1_id=$folderid&del=2'> ยืนยัน</a>
	<a class='btn btn-default' href='showobjhierar.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($del == 2)
...
$sql = "DELETE FROM hierarchical_category WHERE folder1_id = '$folderid' ";
$query=mysqli_query($link,$sql) or die("Can't Querssy");

$sql2 = "DELETE FROM hierarchical_category WHERE folder1_parent = '$folderid' ";
$query2=mysqli_query($link,$sql2) or die("Can't Query2");

// cat lv 2
$sql3 = "DELETE FROM hierarchical_category_lv2 WHERE hc2_id = '$folderid' ";
$query3=mysqli_query($link,$sql3) or die("Can't Query3");

$sql4 = "DELETE FROM hierarchical_category_lv2 WHERE hc1_id = '$folderid' ";
$query4 = mysqli_query($link,$sql4) or die("Can't Query4");

$getlv3 = "SELECT * FROM hierarchical_category_lv3";
$query = mysqli_query($link , $getlv3);

foreach ($query as $rows) {
	$lv3id = $rows['hc3_id'];
	$lv3parant = $rows['hc2_id'];
}
// cat lv 3
$sql5 = "DELETE FROM hierarchical_category_lv3 WHERE hc3_id = '$lv3id' ";
$query5 =mysqli_query($link,$sql5) or die("Can't Query5");

$sql6 = "DELETE FROM hierarchical_category_lv3 WHERE hc2_id = '$lv3id' ";
...
$folder1_name = $_REQUEST['folder1_name'];
$folder1_parent = $_REQUEST['folder1_parent'];
echo "<form class='form-horizontal' role='form' name='form1' action='showobjhierar.php' method='post'>";

$sql = "select * from hierarchical_category where folder1_parent = '0' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$num_rows=mysqli_num_rows($query);
echo "<div class='form-group'>";
echo "<label class='col-sm-2 control-label' >เลือก Folder หลัก</label>";
echo "<div class='col-sm-8'>";
//echo "เลือกประเภทหลัก<select name ='folder1_parent'>";
echo "<select class='form-control' name ='folder1_parent' id='ca' lv='1'>";
echo "<option value='0'>-- สร้างแฟ้มหลัก --</option>";
for ($i=0; $i<$num_rows; $i++) {
         $result=mysqli_fetch_array($query);
         echo "<option value='$result[folder1_id]'>$result[folder1_name]</option>";
}
echo "</select>";
echo "</div>";
...
$sql = "select  count(*) from hierarchical_category  where folder1_parent = '0' ";
$query=mysqli_query($link,$sql) or die("Can't Queดดดry");
$num_rows=mysqli_num_rows($query);
...
//$sql="Select count(*) From hierarchical_object"; //นับจำนวน Record ทั้งหมดใน Table
//$query=mysqli_query($link,$sql) or die("Can't Query");
//$rs=mysqli_query($link,$sql);

//$total_rec=mysql_result($folder1_id,0,0); // เก็บจำนวน Record ทั้งหมดไว้ใน $total_page
...
$sql = "select * from tz_members where id = '$id'   ";
 $query=mysqli_query($link,$sql) or die("Can't Queหหหry");
 $num_rows=mysqli_num_rows($query);
...
////////// End Check User ///////
          ...
          $sql = "SELECT * from hierarchical_category where folder1_parent = '0'  order by folder1_id DESC LIMIT $start , $p_size ";
          $query=mysqli_query($link,$sql) or die("Can't Qหหuery");
          $num_rows=mysqli_num_rows($query);

//$sql="Select * From hierarchical_object LIMIT $start , $p_size";
...
while($c1 = mysqli_fetch_array($rs)) {
	echo "<tr>";
		echo "<form action='showobjhierar.php' method='post'>";
			echo "<td class='text-center'>".++$c1num."</td>";
			echo "<td><input class='form-control' type='text' name='folder1_name' value='".$c1['folder1_name']."'></td>";
			echo "<td class='text-center'>";
				echo "<input class='form-control' type='hidden' name='edit' value='1'>";
				echo "<input type='hidden' name='folder1_id' value='".$c1['folder1_id']."'>";
				echo "<button name='type' class='btn btn-link' type='submit' value='edit'><img src='images/edit_icon2.png'></button>"." ";
				echo "<a class='btn btn-link' href='showobjhierar.php?folder1_id=".$c1['folder1_id']."&del=1'><img src='images/icon_del2.png'></a>";
			echo "</td>";
		echo "</form>";
	echo "</tr>";
	...
	$cmd = "
		SELECT *
		FROM `hierarchical_category_lv2` AS c2
		LEFT JOIN `hierarchical_object` AS c1 ON c2.hc1_id = c1.folder1_id
		WHERE c2.hc1_id = '".$c1['folder1_id']."'
	";
	...
	if(mysqli_num_rows($queryCL2) != 0) {
		...
		while($c2 = mysqli_fetch_assoc($queryCL2)) {
			echo "<tr>";
				echo "<form action='model/funchierarchical.php' method='post'>";
					...
					"<td>
						<div class='input-group col-md-11 col-md-offset-1'>
							...
							<input name='category' type='text' value='".$c2['hc2_name']."' class='form-control' style='background-color: #ccc;'>
						</div>
					</td>";
					echo "<td class='text-center'>";
						...
						echo "<input type='hidden' name='parents' value='".$c2['hc2_id']."'>";
						echo "<input type='hidden' name='table' value='hierarchical'>";
			...
			$cmd = "
				SELECT *
				FROM `hierarchical_category_lv3` AS c3
				LEFT JOIN `hierarchical_category_lv2` AS c2 ON c3.hc2_id = c2.hc2_id
				WHERE c3.hc2_id = '".$c2['hc2_id']."'
			";
						...
						echo "<form action='model/funchierarchical.php' method='post'>";
							...
							<td>
								<div class='input-group col-md-10 col-md-offset-2'>
									<span class='input-group-addon'><i class='fa fa-caret-right'></i></span>
									<input type='text' name='category' value='".$c3['hc3_name']."' class='form-control' style='background-color: #eee;'>
								</div><!-- /.input-group -->
							</td>";
							echo "<td class='text-center'>";
								echo "<button class='btn btn-link' name='type' value='update' type='submit'><img src='images/edit_icon2.png'></button>"." ";
								echo "<button class='btn btn-link' name='type' value='delete' type='submit'><img src='images/icon_del2.png'></button>";
								echo "<input type='hidden' name='id' value='".$c3['hc3_id']."'>";
								echo "<input type='hidden' name='table' value='hierarchical'>";
				...
				$("#alert-message").alert();
					window.setTimeout(function() { $("#alert-message").alert('close'); }, 3000);
						...
						level_1_add += "<br>";
							...
							level_1_add += "<input class='form-control' type='text' name='folder1_name' value ='' required email>";
						level_1_add += "<input class='form-control' type='hidden' name='folder1_parent' value='0'>";
						level_1_add += "<input class='form-control' type='hidden' name='new' value='1'>";
						level_1_add += "</div>";
						level_1_add += "<div class='col-sm-2'>";
						level_1_add += "<br>";
						level_1_add += "<button type='submit' name='submit' class='btn btn-primary'>เพิ่มโฟลเดอร์หลัก</button>";
						level_1_add += "</div>";
						...
						$("#ca").on('change', function() {
							...
							if($(this).val() != 0) {
								...
								level_2_add += "<form method='post' action='h_function_addsub.php?parant="+$(this).val()+"'>";
								level_2_add += "<div class='form-group' style='margin-bottom: 5px !important;'>";
								level_2_add += "<div class='col-md-2 col-sm-offset-2'>";

								level_2_add += "</div><!-- /.col-sm-8 col-sm-offset-2 -->";
								level_2_add += "<div class='col-sm-2'>";
								level_2_add += "<button type='submit' class='btn btn-success' name='add' value='addobj'></button>";
								level_2_add += "</div><!-- /.col-sm-2 -->";
								level_2_add += "<div class='col-sm-2'>";
								level_2_add += "<button type='submit' class='btn btn-success' name='type' value='insert'>เพิ่มโฟลเดอร์ย่อย</button>";
								level_2_add += "</div><!-- /.col-sm-2 -->";
								level_2_add += "</div><!-- /.form-group -->";
								level_2_add += "<input type='hidden' name='table' value='hierarchical'>";
								...
								$.ajax({url: "model/getHierarchical.php?table=hc&level=2&id="+ $(this).val(), success: function(result){
									var fetch = $.parseJSON(result);
									if(fetch.length != 0) {
										level_2_list += "<br>";
										level_2_list += "<form method='post' action='model/funchierarchical.php'>";
										...
										level_2_list += "<input type='hidden' name='table' value='hierarchical'>";
								...
								level_1_add += "<div class='col-sm-2'>";
								level_1_add += "<br>";
								level_1_add += "<button type='submit' name='submit' class='btn btn-primary'>สร้างโฟลเดอร์หลัก</button>";
								level_1_add += "</div>";
