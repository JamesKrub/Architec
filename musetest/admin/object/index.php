...
//             $sql = "SELECT * FROM  archive_object  Where obj_title like '%$s%' group by obj_refcode order by  obj_id DESC Limit 0,20;";
                 $sql = "SELECT * FROM  muse_object  Where obj_title like '%$s%' group by obj_refcode order by  obj_id DESC Limit 0,20;";
			}
			else if ($permission =='user')
			{
			$sql = "SELECT * FROM  muse_object  Where obj_title like '%$s%'  and user_id = '$id' group by obj_refcode order by  obj_id DESC Limit 0,20;";
            }
  $query=mysqli_query($sql,$link) or die("55");
   $num_rows=mysqli_num_rows($query);
...
else
	    {
				if(($permission == 'admin') or ($permission == 'superadmin') or ($permission =='editor'))
			{
				$sql = "SELECT * FROM  muse_object  group by obj_refcode order by  obj_id DESC Limit 0,20;";
			}
			else if ($permission =='user')
			{
				$sql = "SELECT * FROM  muse_object  where user_id ='$id' group by obj_refcode order by  obj_id DESC Limit 0,20;";
			}
...
for ($i=0; $i<$num_rows; $i++) {
	...
	$sql2 = "SELECT * FROM  muse_pic  WHERE obj_refcode = '$piccode' Limit 0,1 ";
		...
		if($line < 4 )
		 {
						...
						echo "<td align='center'> <a href=\"../../pic/bigmuse/$refcode/$result2[pic_name]\" title=\"$result[obj_title]\" data-gallery><img src='../../pic/bigmuse/$refcode/$result2[pic_name]' width='200'></a><br>";
						...
						{
						echo "<a href='editmuse.php?objectid=$result[obj_id]&refcode=$result2[obj_refcode]' > $result[obj_title]</a></td>";
						}
						else
						{
						echo "<a href='editmuse.php?objectid=$result[obj_id]&refcode=$result2[obj_refcode]' > $result[obj_title]</a></td>";
	 ...
	 else
		 {
						 ...
						 echo "<td align='center'><a href=\"../../pic/bigmuse/$refcode/$pic\" title=\"$result[obj_title]\" data-gallery><img src='../../pic/bigmuse/$refcode/$bigpic' width='200'></a><br>";
						 if (($permission == 'editor') and ($result[user_id] != $id))
						 {
