...
$del = $_REQUEST['del'];
$new = $_REQUEST['new'];
$hc2_id = $_REQUEST['hc2_id'];
$hc2_name = $_REQUEST['hc2_name'];
$hc1_id = $_REQUEST['hc1_id'];
...
$sql = "UPDATE hierarchical_category_lv2 SET `hc2_name` = '$hc2_name'  WHERE `hierarchical_category_lv2`.`hc2_id` = '$hc2_id' LIMIT 1 ;";
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='h_function_addsub.php?hc2_id=$hc2_id&del=2'> ยืนยัน</a>
...
// cat lv 2
$sql3 = "DELETE FROM hierarchical_category_lv2 WHERE hc2_id = '$hc2_id' ";
$query3=mysqli_query($link,$sql3) or die("Can't Query3");

$sql4 = "DELETE FROM hierarchical_category_lv2 WHERE hc1_id = '$hc2_id' ";
...
$sql = "INSERT INTO hierarchical_category_lv2 (`hc2_id` ,`hc2_name`,`hc1_id` )VALUES (NULL, '$hc2_name','$hc1_id');";
...
echo "<form class='form-horizontal' role='form' name='form1' action='h_function_addsub.php' method='post'>";

$sql = "select * from hierarchical_category_lv2 where hc1_id = '$parentssub' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$num_rows=mysqli_num_rows($query);
echo "<div class='form-group'>";
echo "<label class='col-sm-2 control-label' >เลือกแฟ้มย่อย</label>";
echo "<div class='col-sm-8'>";
//echo "เลือกประเภทหลัก<select name ='folder1_parent'>";
echo "<select class='form-control' name ='hc1_id' id='ca' lv='2'>";
echo "<option value='0'>-- สร้างแฟ้มย่อย --</option>";
for ($i=0; $i<$num_rows; $i++) {
         $result=mysqli_fetch_array($query);
         echo "<option value='$result[hc2_id]'>$result[hc2_name]</option>";
}
echo "</select>";
echo "</div>";
...
$sql = "select  count(*) from hierarchical_category_lv2  where hc1_id = '$parentssub' ";
...
echo "<thead>";
echo "<tr>";
echo "<th class='col-sm-2 text-center'>ชื่อแฟ้มหลัก</th>";
echo "<th class='col-sm-6 text-center'>แฟ้มย่อย</th>";
...
////////// End Check User ///////
          ...
          $sql = "SELECT * from hierarchical_category join hierarchical_category_lv2 where folder1_parent = '0' AND hc1_id = '$parentssub'  order by folder1_parent DESC LIMIT $start , $p_size ";
          $query=mysqli_query($link,$sql) or die("Can't Qหหuery");
          $num_rows=mysqli_num_rows($query);
...
while($c1 = mysqli_fetch_array($rs)) {
	echo "<tr>";
		echo "<form action='h_function_addsub.php' method='post'>";
			...
			echo "<td><input class='form-control' type='text' name='hc2_name' value='".$c1['hc2_name']."'></td>";
			echo "<td class='text-center'>";
				echo "<input class='form-control' type='hidden' name='edit' value='1'>";
				echo "<input type='hidden' name='hc1_id' value='".$c1['hc1_id']."'>";
				echo "<button name='type' class='btn btn-link' type='submit' value='edit'><img src='images/edit_icon2.png'></button>"." ";
				echo "<a class='btn btn-link' href='h_function_addsub.php?hc1_id=".$c1['folder1_id']."&del=1'><img src='images/icon_del2.png'></a>";
			echo "</td>";
		echo "</form>";
	echo "</tr>";
            ...
            $(function() {
				...
				$("#alert-message").alert();
					window.setTimeout(function() { $("#alert-message").alert('close'); }, 3000);
						...
						level_1_add += "<br>";
						level_1_add += "<button type='submit' name='submit' class='btn btn-primary'>เพิ่มโฟลเดอร์ย่อย</button>";
						level_1_add += "</div>";
						...
						$("#ca").on('change', function() {
							...
							if($(this).val() != 0) {
								...
								level_2_add += "<form method='post' action='h_function_addsub2.php?parant="+$(this).val()+"'>";
								level_2_add += "<div class='form-group' style='margin-bottom: 5px !important;'>";
								level_2_add += "<div class='col-sm-3 col-sm-offset-2'>";
