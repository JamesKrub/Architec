...
// $sql1111 = "select * from hierarchical_category where folder1_id = '$parentssub' ";
// $query111=mysqli_query($link,$sql1111) or die("Can't Query");
//
//
// while($c2 = mysqli_fetch_assoc($query111)) {
//   echo $c2['folder1_name'];
// }
...
$sql = "UPDATE hierarchical_category SET `folder1_name` = '$folder1_name'  WHERE `hierarchical_category`.`folder1_id` = '$folder1_id' LIMIT 1 ;";
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='h_function_addsub.php?folder1_id=$folder1_id&del=2'> ยืนยัน</a>
	<a class='btn btn-default' href='h_function_addsub.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($del == 2)
...
// cat lv 2
$sql3 = "DELETE FROM hierarchical_category WHERE folder1_id = '$folder1_id' ";
$query3=mysqli_query($link,$sql3) or die("Can't Query3");

$sql4 = "DELETE FROM hierarchical_category WHERE folder1_parent = '$folder1_id' ";
...
echo "<form class='form-horizontal' role='form' name='form1' action='h_function_controller.php' method='post'>";
$sql = "select * from hierarchical_category where folder1_parent = '0' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$num_rows=mysqli_num_rows($query);
echo "<div class='form-group'>";
echo "<label class='col-sm-2 control-label' >เลือกแฟ้ม</label>";
echo "<div class='col-sm-8'>";
// echo "เลือกประเภทหลัก<select name ='folder1_parent'>";

echo "<select class='form-control' name ='folder1_id' id='ca' lv='1'>";
echo "<option value='0'>-- สร้างแฟ้มใหม่ --</option>";
for ($i=0; $i<$num_rows; $i++) {
         $result=mysqli_fetch_array($query);
...
$sql = "select  count(*) from hierarchical_category  where folder1_parent = '$parentssub' ";
...
echo "<th class='col-sm-6 text-center'>แฟ้มหลัก</th>";
...
////////// End Check User ///////
          $sql = "SELECT distinct * from hierarchical_category where folder1_parent = '$parentssub'  order by folder1_parent DESC LIMIT $start , $p_size ";
          $query=mysqli_query($link,$sql) or die("Can't Qหหuery");
          $num_rows=mysqli_num_rows($query);
...
$resultdata=mysqli_fetch_array($query);
	...
	echo "<tr>";
		echo "<form action='h_function_addsub.php' method='post'>";
			echo "<td class='text-center'>".++$c1num."</td>";
			echo "<td><input class='form-control' type='text' name='folder1_name' value='".$resultdata['folder1_name']."'></td>";
			echo "<td class='text-center'>";
				echo "<input class='form-control' type='hidden' name='edit' value='1'>";
				echo "<input type='hidden' name='folder1_parent' value='".$resultdata['folder1_parent']."'>";
				echo "<button name='type' class='btn btn-link' type='submit' value='edit'><img src='images/edit_icon2.png'></button>"." ";
				echo "<a class='btn btn-link' href='h_function_addsub.php?folder1_id=".$resultdata['folder1_id']."&del=1'><img src='images/icon_del2.png'></a>";
			echo "</td>";
		echo "</form>";
	echo "</tr>";
	...
	$cmd = "
		SELECT *
		FROM `hierarchical_category` AS c2
		LEFT JOIN `hierarchical_object` AS c1 ON c2.folder1_parent = c1.folder1_id
		WHERE c2.folder1_parent = '".$c1['folder1_id']."'
	";
				...
				echo "<form action='model/funchierarchical.php' method='post'>";
					...
					"<td>
						<div class='input-group col-md-11 col-md-offset-1'>
							...
							<input name='category' type='text' value='".$c2['folder1_name']."' class='form-control' style='background-color: #ccc;'>
						</div>
					</td>";
					echo "<td class='text-center'>";
						...
						echo "<input type='hidden' name='parents' value='".$c2['folder1_id']."'>";
            ...
            $(function() {
				...
				$("#alert-message").alert();
					window.setTimeout(function() { $("#alert-message").alert('close'); }, 3000);
						...
						level_1_add += "<input class='form-control' type='hidden' name='type' value='add'>";
						level_1_add += "</div>";
						level_1_add += "<div class='col-sm-2'>";
						level_1_add += "<br>";
						level_1_add += "<button type='submit' name='submit' value='adddata' class='btn btn-primary'>เพิ่มแฟ้ม</button>";
						level_1_add += "</div>";
						...
						$("#ca").on('change', function() {
							...
							if($(this).val() != 0) {
								...
								level_2_add += "<form method='post' action='h_function_controller.php?parant="+$(this).val()+"'>";
								...
								level_2_add += "<button type='submit' class='btn btn-success' name='type' value='addobj'>เพิ่มจดหมายเหตุ</button>";
								level_2_add += "</div><!-- /.col-sm-2 -->";
								level_2_add += "<button type='submit' class='btn btn-success' name='type' value='insert'>เพิ่มแฟ้ม</button>";
								level_2_add += "</div><!-- /.col-sm-4 -->";
								...
								level_1_add += "<div class='col-sm-2'>";
								level_1_add += "<br>";
								level_1_add += "<button type='submit' name='submit' class='btn btn-primary'>เพิ่มแฟ้ม</button>";
								level_1_add += "</div>";
