                    ...
                    <ul class="nav navbar-nav">
                        ...
                        <li class="dropdown user user-menu">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                ...
                                <span> <?php echo $_SESSION['usr'] ?$_SESSION['usr'] : 'Admin';?> <i class="caret"></i></span>
                            </a>
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-header bg-light-blue">
									...
									?>
                                    <p>
                                        <?php echo $_SESSION['usr'] ?$_SESSION['usr'] : 'Admin';?>
                                        <small>
											<?php
											if($_SESSION['id'])
											{
...
$sql6 = " SELECT `permission` from `tz_members` where id = '1' "; 
...
////////// Check User /////////
$sql13 = "SELECT * FROM tz_members WHERE id = '$id' ";
$result13 = mysqli_query($link,$sql13) or die("1");
$objnum = mysqli_num_rows($result13);
        

        
$sql14 = " SELECT `permission` from `tz_members` where id = '1' "; 
            
            $query14 =  mysqli_query($link,$sql14);
            while($row14 = mysqli_fetch_array($query14,MYSQLI_ASSOC))
            {
                $permission= $row14['permission'];
               // $osm_lname= $row1['LNAME'];
            }
////////// End Check User ///////
...
//echo "Per : $permission <br>";
//	$data = array();
//	$data['query']['muse'] = mysqli_query("SELECT * FROM `muse_object` AS o LEFT JOIN `muse_pic` AS p ON o.obj_refcode = p.obj_refcode WHERE p.obj_cover = 1 LIMIT 8");
        $sql9 = "SELECT * FROM `muse_object` LEFT JOIN `muse_pic` ON `muse_object`.`obj_refcode` = `muse_pic`.`obj_refcode` WHERE `muse_pic`.`obj_cover` = '1' LIMIT 0,8 ";
...
?>
				...
				<div class="col-md-3 text-center">
					<a href="../../pic/bigmuse/<?php echo $refcode."/".$bigpic ?>" title="<?php $result10['obj_title']; ?>" data-gallery><img style="height: 135px !important;" src="../../pic/thumbmuse/<?php echo $refcode."/".$thumb ?>"></a>
					<br>
					<a href="editmuse.php?objectid=<?php echo $result10['obj_id']; ?>&refcode=<?php echo $refcode; ?>"><?php echo $result10['obj_title']; ?></a>
				</div><!-- /.col-md-3 -->
			<?php } ?>
		</div>
	</div>
