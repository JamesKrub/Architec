...
$sql = "UPDATE archive_category SET `cat1_name` = '$catname'  WHERE `archive_category`.`cat1_id` = '$catid' LIMIT 1 ;";
$query=mysqli_query($link,$sql) or die("Can't Query");
...
<div class='callout callout-danger'>
	...
	<p>
	<a class='btn btn-danger' href='showobjcat.php?catid=$catid&del=2'> ยืนยัน</a>
	<a class='btn btn-default' href='showobjcat.php'> ยกเลิก</a>
	</p>
</div>";
}
else if ($del == 2)
...
$sql = "DELETE FROM archive_category WHERE cat1_id = '$catid' ";
$query=mysqli_query($link,$sql) or die("Can't Query");

$sql2 = "DELETE FROM archive_category WHERE cat1_parent = '$catid' ";
$query2=mysqli_query($link,$sql2) or die("Can't Query2");

// cat lv 2
$sql3 = "DELETE FROM archive_category_lv2 WHERE ac2_id = '$catid' ";
$query3=mysqli_query($link,$sql3) or die("Can't Query3");

$sql4 = "DELETE FROM archive_category_lv2 WHERE ac1_id = '$catid' ";
$query4 = mysqli_query($link,$sql4) or die("Can't Query4");

$getlv3 = "SELECT * FROM archive_category_lv3";
...
// cat lv 3
$sql5 = "DELETE FROM archive_category_lv3 WHERE ac3_id = '$lv3id' ";
$query5 =mysqli_query($link,$sql5) or die("Can't Query5");

$sql6 = "DELETE FROM archive_category_lv3 WHERE ac2_id = '$lv3id' ";
...
$sql = "INSERT INTO archive_category (`cat1_id` ,`cat1_name`,`cat1_parent` )VALUES (NULL, '$catname','$parentid');";
$query=mysqli_query($link,$sql) or die("Can't Query");
...
echo "<form class='form-horizontal' role='form' name='form1' action='showobjcat.php' method='post'>";
...
$sql = "select  count(*) from archive_category  where cat1_parent = '0' ";
$query=mysqli_query($link,$sql) or die("Can't Query");
$num_rows=mysqli_num_rows($query);
...
////////// End Check User ///////
          ...
          $sql = "select * from archive_category where cat1_parent = '0'  order by cat1_id DESC LIMIT $start , $p_size ";
          $query=mysqli_query($link,$sql) or die("Can't Query");
          $num_rows=mysqli_num_rows($query);

//$sql="Select * From archive_category LIMIT $start , $p_size";
...
while($c1 = mysqli_fetch_array($rs)) {
	echo "<tr>";
		echo "<form action='showobjcat.php' method='post'>";
			...
			echo "<td class='text-center'>";
				...
				echo "<a class='btn btn-link' href='showobjcat.php?catid=".$c1['cat1_id']."&del=1'><img src='images/icon_del2.png'></a>";
			echo "</td>";
		echo "</form>";
	echo "</tr>";
	...
	$cmd = "
		SELECT *
		FROM `archive_category_lv2` AS c2
		LEFT JOIN `archive_category` AS c1 ON c2.ac1_id = c1.cat1_id
	...
	if(mysqli_num_rows($queryCL2) != 0) {
		...
		while($c2 = mysqli_fetch_assoc($queryCL2)) {
			echo "<tr>";
				echo "<form action='model/funcCategory.php' method='post'>";
					...
					echo "<td class='text-center'>";
						...
						echo "<input type='hidden' name='table' value='archive'>";
			...
			$cmd = "
				SELECT *
				FROM `archive_category_lv3` AS c3
				LEFT JOIN `archive_category_lv2` AS c2 ON c3.ac2_id = c2.ac2_id
					...
					window.setTimeout(function() { $("#alert-message").alert('close'); }, 3000);
						...
						$("#ca").on('change', function() {
							...
							if($(this).val() != 0) {
								...
								level_2_add += "<input type='hidden' name='table' value='archive'>";
								...
								$.ajax({url: "model/getCategory.php?table=arc&level=2&id="+ $(this).val(), success: function(result){
									...
									if(fetch.length != 0) {
										...
										level_2_list += "<input type='hidden' name='table' value='archive'>";
