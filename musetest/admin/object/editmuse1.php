...
######  ADD DB############
$sql = " INSERT INTO `muse_vr` ( `vr_id` , `vr_dir` , `obj_refcode`, `vr_direction` ) VALUES ('' , '$newdir1', '$refcode','R');";
$query=mysqli_query($link,$sql) or die("Can't Query");
}
...
echo "<div class='box box-primary'>"; // Start box-primary
    ...
    echo "<a href='editmuse.php?deldownload=1&objectid=$objectid&refcode=$refcode'><img src='images/icon_del2.png'> </a>ลบ <Br><br>";
      ...
      if($dwload == 1)
		{
	      $sql = "DELETE FROM muse_upload WHERE bpu_id = '$bpuid' ";
		  $query=mysqli_query($link,$sql) or die("Can't Query");
         echo "ลบภาพ 3 มิติเสร็จสิ้น";
		 echo "<br>";
		}
		...
		";
			...
			while($row = mysql_fetch_assoc($ul_query)) {
				...
				echo "
					...
					<p>
						<a href='editmuse.php?deldwload=1&objectid=$objectid&bpuid=$row[bpu_id]'><img src='images/icon_del2.png'> </a>ลบ $row[bpu_id]
                        <a href ='editmuse.php?objectid=$objectid&refcode=$refcode&bpuid=$row[bpu_id]&dwload=1'> <img src ='images/icon_del.gif'> </a>
					</p>
				";
				echo "</div><!-- /.col-md-2 -->";
                if($deldwload == 1)
...
#########ลอง
    echo "<div class='box box-primary'>"; // Start box-primary
	echo "<div class='box-header'>";
	echo "<h3 class='box-title'>ไฟล์</h3>";
	echo "</div>";
	echo "<div class='box-body'>"; // Start box-body
	echo "<div>
			<a class='btn btn-primary' href=\"JavaScript:newPopup('uploadfile/index.php?dir=$refcode&objectid=$objectid&refcode=$refcode');\">
				<i class='fa fa-upload fa-lg'></i> อัพโหลดไฟล์ดาวน์โหลด
			</a>
			<a class='btn btn-primary pull-right' href=\"JavaScript:newPopup('model/sortable/index.php?ref=$refcode&type=muse&id=$objectid');\">
				<i class='fa fa-sort-amount-desc fa-lg'></i> จัดเรียง
			</a><br><br>
            </div>";
	echo "</div>";
   echo "</div>";
...
{
//echo "สั่งเปิดรูป";
 $sql = "UPDATE muse_pic SET `pic_open`='0' WHERE `muse_pic`.`pic_id` =$picid";
 $query=mysqli_query($link,$sql) or die("Can't Query-1");
}
else if($openpic == '0')
{
//echo "สั่งปิดรูป";
 $sql = "UPDATE muse_pic SET `pic_open`='1' WHERE `muse_pic`.`pic_id` =$picid";
 $query=mysqli_query($link,$sql) or die("Can't Query-1");
}
##### Start Tab Pic 1 #######
...
######## UPload FILE###########
 $target_path = "pic/bigmusemuse/";
 $target_path_thumb = "pic/thumbmusemuse/";
...
$original = "pic/bigmusemuse/".$myfile;
$dest = "pic/thumbmusemuse/".$myfile;
copy($original,$dest);

$thumb = "thumb-".$myfile;
$mythumb = "pic/thumbmusemuse/".$thumb;
//echo "$thumb --> $mythumb";
rename($dest, $mythumb);
...
echo "<tr>";
 $sql3 = "SELECT * FROM `muse_pic` WHERE obj_refcode = '$refcode' ORDER BY listorder ";
         $query3=mysqli_query($link,$sql3) or die("Can't Query2");
         $num_rows3=mysqli_num_rows($query3);
	...
	//	echo "x $num_rows3 x <br>";
	     for ($i=0; $i<$num_rows3; $i++) {
			...
			echo "
			<a href=\"JavaScript:newPopupdetail('zoommuse/picdetail.php?picid=$result3[pic_id]&objectid=$objid&refcode=$refcode');\"> <img src='images/edit_icon2.png'></a>
			...
			$line = $line+1;
			 ...
			 else
             {
				 echo "<td align='center'>";
                //echo "<img src ='../../pic/thumbmuse/thumbmuse-$result3[pic_name]' width='50'> $result3[pic_id] $result3[obj_refcode]$result3[pic_name] <br>";
				// echo "        <a href=\"pic/bigmuse/$refcode/$result3[pic_name]\"   media=\"gallery\" id=\"tre\"  class=\"pirobox_gall\" title=\"\">
				//<img src ='../../pic/bigmuse/$refcode/$result3[pic_name]' width='150'></a>
				//<br>
				...
				//<a href ='editmuse.php?picid=$result3[pic_id]&delpic=1&objectid=$objid&refcode=$refcode'> <img src ='images/icon_del.gif'> </a>";
